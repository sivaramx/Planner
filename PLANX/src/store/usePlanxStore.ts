...
function load():AppData{try{const x=localStorage.getItem(KEY);return x?{...defaults,...JSON.parse(x),settings:{...defaults.settings,...JSON.parse(x).settings}}:defaults}catch{return defaults}}
const save=(d:AppData)=>{try{localStorage.setItem(KEY,JSON.stringify(d))}catch(e){console.warn("PLANX storage error",e)}};
...
interface Store extends AppData{
 ...
 clearAll:()=>void; replaceData:(d:AppData)=>void; clearDemo:()=>void;
}
const loaded=load();
export const usePlanxStore=create<Store>((set)=>({
 ...
 duplicateTask:id=>set(s=>{const t=s.tasks.find(x=>x.id===id);if(!t)return s;const d={...s,tasks:[...s.tasks,{...t,id:uid(),title:`${t.title} (copy)`,completed:false,status:"todo",completedAt:undefined,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),order:s.tasks.length}]};save(d);return d}),
