...
export default function App(){
 ...
 const theme=usePlanxStore(s=>s.theme),animations=usePlanxStore(s=>s.settings.animations);
 ...
 useEffect(()=>{const open=()=>setModal(true),key=(e:KeyboardEvent)=>{const target=e.target as HTMLElement;if(target?.tagName==="INPUT"||target?.tagName==="TEXTAREA")return;if(e.key.toLowerCase()==="n"){e.preventDefault();setModal(true)}if(e.key==="/"){e.preventDefault();setPage("Tasks");setTimeout(()=>document.querySelector<HTMLInputElement>(".search-box input")?.focus(),30)}if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="k"){e.preventDefault();setCmd(true)}if(e.key.toLowerCase()==="t")setPage("Today");if(e.key.toLowerCase()==="c")setPage("Calendar");if(e.key.toLowerCase()==="p")setPage("Projects");if(e.key==="Escape"){setSidebar(false);setCmd(false);setModal(false);setFocus(false)}};addEventListener("planx:add-task",open);addEventListener("keydown",key);return()=>{removeEventListener("planx:add-task",open);removeEventListener("keydown",key)}},[]);
 let content=page==="Dashboard"?<Dashboard setPage={setPage}/>:page==="Tasks"?<Tasks/>:page==="Today"?<Today/>:page==="Calendar"?<CalendarPage/>:page==="Projects"?<Projects/>:page==="Goals"?<Goals/>:page==="Habits"?<Habits/>:page==="Notes"?<Notes/>:page==="Completed"?<Completed/>:page==="Archive"?<Archive/>:<Settings/>;
